...
describe('Topological sorter', () => {
    ...
    it('should accept single fact', () => {
        const sorter = givenTopologicalSorter();
        const sorted = whenSort(sorter, (0, _src_1.dehydrateFact)({
            type: 'Singleton'
        }));
    ...
    it('should handle a real world example', () => {
        ...
        expect(last.fields.value).toEqual("one\ntwo\nthree\nfour\nfive\n\nsix");
    });
});
function givenTopologicalSorter() {
    return new _src_2.TopologicalSorter();
}
function whenSort(sorter, facts) {
    return sorter.sort(facts, (predecessors, fact) => fact);
}
//# sourceMappingURL=sorterSpec.js.map