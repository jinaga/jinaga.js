...
class Player {
    constructor() {
        this.type = "GameHub.Player";
    }
}
Player.Type = "GameHub.Player";
class Playground {
    constructor() {
        this.type = "GameHub.Playground";
    }
}
Playground.Type = "GameHub.Playground";
class Join {
    constructor() {
        this.type = "GameHub.Join";
    }
}
Join.Type = "GameHub.Join";
describe("Disconnected specification detection", () => {
    ...
    it("detects disconnected specification with isolated given", () => {
        ...
        expect(() => {
            ...
            (0, _src_1.invertSpecification)(spec);
        }).toThrow(_src_1.DisconnectedSpecificationError);
    });
    ...
    it("provides clear error message for disconnected specifications", () => {
        try {
            ...
            expect(error).toBeInstanceOf(_src_1.DisconnectedSpecificationError);
    ...
    it("detects disconnected specifications when using Model API with match()", () => {
        ...
        expect(() => {
            ...
            const spec = model.given(Player, Playground).match((player, playground, r) => r.ofType(Join).join(j => j.player, player));
            (0, _src_1.invertSpecification)(spec.specification);
        }).toThrow(_src_1.DisconnectedSpecificationError);
    });
    it("allows connected specifications when using Model API", () => {
        ...
        expect(() => {
            ...
            model.given(Player).match((player, r) => r.ofType(Join).join(j => j.player, player));
        }).not.toThrow();
    });
});
//# sourceMappingURL=disconnectedSpec.js.map